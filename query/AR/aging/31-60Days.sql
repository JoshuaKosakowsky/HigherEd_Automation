/* Aging dashboard card: positive balances aged 31 through 60 days. */
SELECT 
coalesce(SUM(
CASE WHEN (STRPOS('bill',lower({{p_DateColumnType}})) > 0) THEN
(case when ({{p_RunDate}}::date - bill_date::date)>=31 and ({{p_RunDate}}::date - bill_date::date)<=60 then balance end)
WHEN (STRPOS('due',lower({{p_DateColumnType}})) > 0) THEN
(case when ({{p_RunDate}}::date - due_date::date)>=31 and ({{p_RunDate}}::date - due_date::date)<=60 then balance end)
WHEN (STRPOS('effective',lower({{p_DateColumnType}})) > 0) THEN
(case when ({{p_RunDate}}::date - effective_date::date)>=31 and ({{p_RunDate}}::date - effective_date::date)<=60 then balance end)
